...
export default class UploadFilter extends BaseResource {
  ...
  create(body: SimpleSchemaTypes.UploadFilterCreateSchema) {
    return this.rawCreate(
      serializeRequestBody<SchemaTypes.UploadFilterCreateSchema>({
        body,
        type: UploadFilter.TYPE,
      }),
    ).then((body) =>
      deserializeResponseBody<SimpleSchemaTypes.UploadFilterCreateTargetSchema>(
        body,
      ),
    );
  }
  ...
  update(
    ...
    return this.rawUpdate(
      toId(uploadFilterId),
      serializeRequestBody<SchemaTypes.UploadFilterUpdateSchema>({
        body,
        id: toId(uploadFilterId),
        type: UploadFilter.TYPE,
      }),
    ).then((body) =>
      deserializeResponseBody<SimpleSchemaTypes.UploadFilterUpdateTargetSchema>(
        body,
      ),
    );
  }
  ...
  list() {
    return this.rawList().then((body) =>
      deserializeResponseBody<SimpleSchemaTypes.UploadFilterInstancesTargetSchema>(
        body,
      ),
    );
  }
  ...
  find(uploadFilterId: string | SimpleSchemaTypes.UploadFilterData) {
    return this.rawFind(toId(uploadFilterId)).then((body) =>
      deserializeResponseBody<SimpleSchemaTypes.UploadFilterSelfTargetSchema>(
        body,
      ),
    );
  }
  ...
  destroy(uploadFilterId: string | SimpleSchemaTypes.UploadFilterData) {
    return this.rawDestroy(toId(uploadFilterId)).then((body) =>
      deserializeResponseBody<SimpleSchemaTypes.UploadFilterDestroyTargetSchema>(
        body,
      ),
    );
  }
