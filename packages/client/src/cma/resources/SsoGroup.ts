...
export default class SsoGroup extends BaseResource {
  ...
  list() {
    return this.rawList().then((body) =>
      deserializeResponseBody<SimpleSchemaTypes.SsoGroupInstancesTargetSchema>(
        body,
      ),
    );
  }
  ...
  copyRoles(ssoGroupId: string | SimpleSchemaTypes.SsoGroupData) {
    return this.rawCopyRoles(toId(ssoGroupId)).then((body) =>
      deserializeResponseBody<SimpleSchemaTypes.SsoGroupCopyRolesTargetSchema>(
        body,
      ),
    );
  }
  ...
  update(
    ...
    return this.rawUpdate(
      toId(ssoGroupId),
      serializeRequestBody<SchemaTypes.SsoGroupUpdateSchema>({
        body,
        id: toId(ssoGroupId),
        type: SsoGroup.TYPE,
      }),
    ).then((body) =>
      deserializeResponseBody<SimpleSchemaTypes.SsoGroupUpdateTargetSchema>(
        body,
      ),
    );
  }
  ...
  destroy(ssoGroupId: string | SimpleSchemaTypes.SsoGroupData) {
    return this.rawDestroy(toId(ssoGroupId)).then((body) =>
      deserializeResponseBody<SimpleSchemaTypes.SsoGroupDestroyTargetSchema>(
        body,
      ),
    );
  }
